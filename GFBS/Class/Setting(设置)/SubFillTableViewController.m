...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = self.tableType;
...
- (void)setUpArray {
    if ([self.tableType isEqualToString:@"Telephone Number"]) {
...
- (void)okButtonClicked {
    if (_phoneField1.text != NULL) {
        [_thisRestaurant.phone addObject:_phoneField1.text];
    }
...
{
    NSLog(@"fillVC pass data %@", _thisRestaurant.phone[0]);
    [_delegate passValue:_thisRestaurant];
}
