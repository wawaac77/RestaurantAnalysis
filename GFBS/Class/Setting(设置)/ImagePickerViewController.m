...
//  GFBS
//
//  Created by Alice Jin on 10/10/2017.
...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = ZBLocalized(self.type, nil);
    //右边
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"Upload" style:UIBarButtonItemStyleDone target:self action:@selector(updateRestaurant)];
    
    self.view.backgroundColor = [UIColor whiteColor];
    
    if ([self.type isEqualToString:@"Restaurant Banner"]) {
        _maxImageNum = 1;
        _columNum = 4;
        
        UILabel *remindLabel = [[UILabel alloc] initWithFrame:CGRectMake(10 , GFScreenHeight - 400, GFScreenWidth - 20, 30)];
        remindLabel.font = [UIFont systemFontOfSize:14];
        remindLabel.text = ZBLocalized(@"You could only upload one photo for restaurant banner.", nil);
        //[self.collectionView addSubview:remindLabel];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row == _selectedPhotos.count) {
        ...
        imagePickerVc.maxImagesCount = _maxImageNum;
...
- (void)pushTZImagePickerController {
    ...
    TZImagePickerController *imagePickerVc = [[TZImagePickerController alloc] initWithMaxImagesCount:_maxImageNum columnNumber:_columNum delegate:self pushPhotoPickerVc:YES];
...
#pragma mark - Upload Images or Menu
...
- (void)updateRestaurant {
    ...
    NSLog(@"go to update restaurant api");
    //取消请求
    ...
    NSMutableArray *photosInfoArray = [[NSMutableArray alloc] init];
    
    for (int i = 0; i < _selectedPhotos.count; i ++) {
        NSString *imageBase64 = [UIImagePNGRepresentation(_selectedPhotos[i])
        ...
        [photosInfoArray addObject:imageInfo];
    }
    
    NSDictionary *inSubData = [[NSDictionary alloc] init];
    if ([self.type isEqualToString:@"Upload Photos"]) {
        inSubData = @{@"restaurantId": [ZZMyRestaurant shareRestaurant].myRestaurant.restaurantId,
                      @"images": photosInfoArray};
    }
    else if ([self.type isEqualToString:@"Upload Menu"]) {
        inSubData = @{@"restaurantId": [ZZMyRestaurant shareRestaurant].myRestaurant.restaurantId,
                      @"menus": photosInfoArray};
    }
    else if ([self.type isEqualToString:@"Restaurant Banner"]) {
        inSubData = @{@"restaurantId": [ZZMyRestaurant shareRestaurant].myRestaurant.restaurantId,
                      @"banner": photosInfoArray[0]};
    }

    NSDictionary *inData = @{@"action" : @"updateRestaurant",
                             @"token" : userToken,
                             @"data" : inSubData};
    ...
    NSDictionary *parameters = @{@"data" : inData};
     ...
     NSLog(@"go to update restaurant api 2 %@", parameters);
    
    [[GFHTTPSessionManager shareManager] POSTWithURLString:GetURL parameters:parameters success:^(id data) {
        
        UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:@"ZUZU" message:@"Images uploaded!" delegate:self cancelButtonTitle:@"Ok" otherButtonTitles:nil, nil];
        [alertView show];
        NSLog(@"update restaurant sucess");
