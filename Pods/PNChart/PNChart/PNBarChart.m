...
- (void)__addYCoordinateLabelsValues{
  ...
  [self processYMaxValue];
    ...
    //float sectionHeight = (_yHeight - _chartMarginTop - _chartMarginBottom - kXLabelHeight) / _yLabelSum;
    
  
    NSLog(@"PNBarchart* sectionHeight ****************  %zd", sectionHeight);
    //NSLog(@"PNBarchart*  self.frame.size.height %zd", self.frame.size.height);
    //NSLog(@"PNBarchart*  _yHeight %zd", _yHeight);
    NSLog(@"PNBarchart* _chartMarginTop %f", _chartMarginTop);
    NSLog(@"PNBarchart* _chartMarginBottom %f", _chartMarginBottom);
    NSLog(@"PNBarchart* kXLabelHeight %d", kXLabelHeight);
    NSLog(@"PNBarchart* _yLabelSum %ld", (long)_yLabelSum);
  ...
  for (int i = 0; i <= _yLabelSum; i++) {
    ...
    NSLog(@"PNBarchart ----------------- i %zd", i);
      NSLog(@"PNBarchart sectionHeight %zd", sectionHeight);
      NSLog(@"PNBarchart _chartMarginTop %f", _chartMarginTop);
      NSLog(@"PNBarchart kYLabelHeight %d", kYLabelHeight);
      NSLog(@"PNBarchart _yChartLabelWidth %f", _yChartLabelWidth);
      NSLog(@"PNBarchart kYLabelHeight %d", kYLabelHeight);
    ...
    if (_showLabel) {
        ...
        for (int index = 0; index < _xLabels.count; index++) {
            ...
            if (labelAddCount == _xLabelSkip) {
                ...
                label.center = CGPointMake(labelXPosition, self.frame.size.height - kXLabelHeight - _chartMarginTop + label.frame.size.height /2.0 + _labelMarginTop);
                
                //changed by Alice
                //label.center = CGPointMake(labelXPosition, _yHeight - kXLabelHeight - _chartMarginTop + label.frame.size.height /2.0 + _labelMarginTop);
                labelAddCount = 0;
...
- (void)updateBar
{
    ...
    //CGFloat chartCavanHeight = _yHeight - _chartMarginTop - _chartMarginBottom - kXLabelHeight;
    //change by Alice
    NSInteger index = 0;
